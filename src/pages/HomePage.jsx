        ...
        <div className="max-w-7xl mx-auto px-5 md:px-10 w-full">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center py-16 lg:py-20">
            ...
            <div className="flex flex-col justify-center order-2 lg:order-1">
              ...
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-light leading-snug mb-6" style={{ color: '#0F172A', letterSpacing: '-0.5px' }}>
                {hero.headline}
                <br />
                <span className="font-medium">{hero.headlineSub}</span>
              </h1>
              <div className="h-px w-16 mb-6" style={{ backgroundColor: '#E5E7EB' }} />
              <p className="text-base md:text-lg leading-relaxed mb-10 max-w-lg" style={{ color: '#64748B', fontWeight: 300 }}>
                {hero.body}
              </p>
              ...
              <div className="flex items-center gap-6 mt-12 pt-8" style={{ borderTop: '1px solid #E5E7EB' }}>
                {['דיסקרטיות מלאה', 'יחס אישי', 'מקצועיות ויסודיות'].map((t, i, arr) => (
                  <div key={t} className="flex items-center gap-6">
                    ...
                    {i < arr.length - 1 && <div className="w-px h-6" style={{ backgroundColor: '#E5E7EB' }} />}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="order-1 lg:order-2 flex justify-center lg:justify-start">
              <div className="relative">
                ...
                <div className="absolute bottom-8 right-0 px-5 py-3 z-20" style={{ backgroundColor: 'rgba(15,23,42,0.9)' }}>
        ...
        <div className="max-w-7xl mx-auto px-5 md:px-10 py-12">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px" style={{ backgroundColor: 'rgba(255,255,255,0.08)' }}>
            {values.map((item, i) => (
              <div
                key={i}
                className="flex flex-col items-start p-8 group transition-all duration-300"
                style={{ backgroundColor: '#0F172A' }}
              ...
              >
                ...
                <p className="text-sm leading-relaxed font-light" style={{ color: '#FFFFFF' }}>{item.body}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {processSteps.map((step, i) => (
              <div key={i} className="flex flex-col items-start group">
                ...
                <h3 className="text-base font-semibold mb-3" style={{ color: '#0F172A' }}>{step.title}</h3>
                <div className="h-px w-8 mb-3 transition-all duration-300 group-hover:w-12" style={{ backgroundColor: '#516A73' }} />
                <p className="text-sm leading-relaxed font-light" style={{ color: '#64748B' }}>{step.body}</p>
              </div>
            ))}
          </div>
        </div>
